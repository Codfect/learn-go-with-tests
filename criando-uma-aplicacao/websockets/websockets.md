...
Nosso servidor tem um router setup, então é relativamente simples de consertar.

Adicione ao nosso router
...
Você também vai notar que eu mudei `checaStatus` para aceitar `resposta` em vez
de `resposta.Codigo` porque sinto que a leitura é mais simples.

Agora devemos fazer com que o nosso endpoint retorno algum HTML. Aqui está
